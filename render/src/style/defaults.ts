...
/** `case` applied to a word. `as-written` is the identity, and is the default so
 *  a style must opt in to shouting. */
export function applyCase(text: string, textCase: CaptionStyle["case"]): string {
  switch (textCase) {
    case "upper":
